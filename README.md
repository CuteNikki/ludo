...
- **The full game:** captures, the home column, a move timer, automatic single moves, optional "must spawn on six", "extra turn on capture", "safe start squares" and "three tries" rules and a fair dice option, so no one waits forever for a six.
...
The creator of a room is the host and can configure the turn timer (15, 30, 45 or 60 seconds), automatic moves, the fair dice option, the optional "must spawn on six", "extra turn on capture", "safe start squares" and "three tries" rules and the room's public/private visibility in the lobby. Settings are synchronized to all players and locked once the game starts. Any change made in the lobby resets every player's ready status. Public rooms show up, while still in the lobby, on the `/discover` page for anyone to browse and join, along with a per-setting overview (on/off and move time) of how the room is configured.
